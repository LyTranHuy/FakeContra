...
bool InitData()
{
	...
	//Create window
	g_window = SDL_CreateWindow("Fake Contra",
	...
	g_sound_bullet[1] = Mix_LoadWAV("sound//thread_sound_bullet.wav");
	g_sound_explosion[0] = Mix_LoadWAV("sound//death.wav");
	g_sound_explosion[1] = Mix_LoadWAV("sound//bullet-hit.wav");
	g_revival = Mix_LoadWAV("sound//revival.wav");
	g_music = Mix_LoadMUS("sound//background_music.mp3");
	if (g_sound_bullet[0] == NULL || g_sound_bullet[1] == NULL || g_sound_explosion[0] == NULL || g_sound_explosion[1] == NULL || g_revival == NULL || g_music == NULL)
	{
		success = false;
	}
...
bool LoadVictoryPose()
{
	bool ret = g_victory.LoadImg("img//victory_pose.png", g_screen);
	if (ret = false)
	{
		return false;
	}
...
std::vector<ThreatsObject*> MakeThreadList()
{
	...
	for (int i = 0; i < 15; i++)
	{
		...
		if (thread_obj3 != NULL)
		{
			thread_obj3->LoadImg("img//shooter_threat.png", g_screen);
...
int main(int argc, char* argv[])
{
	...
	while (!quit)
	{
		...
		while (SDL_PollEvent(&g_event) != 0)
		{
			...
			p_player.HandleInputAction(g_event, g_screen, g_sound_bullet[0]);
		}
